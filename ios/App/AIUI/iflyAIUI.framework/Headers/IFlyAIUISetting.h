 ...
 * @param val
 */
+ (void) setSystemInfo: (NSString *)key withVal: (NSString *)val;
 ...
 * @return
 */
+ (NSString *) getSystemInfo: (NSString *)key;

/*!
 * 设置经纬度。参数优先级：CMD_WRITE消息params参数中携带 > aiui.cfg的audioparams中设置 > setGpsPos方法设置。
 * @param lng
 * @param lat
 */
+ (void) setGpsLng: (float )lng andLat: (float )lat;
