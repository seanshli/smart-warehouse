...
export default function RoomManagement() {
  ...
  const fetchRooms = async () => {
    try {
      ...
      const response = await fetch(`/api/warehouse/rooms${params}`)
      if (response.ok) {
  ...
  const handleRoomClick = async (room: Room) => {
    try {
      ...
      const response = await fetch(`/api/warehouse/rooms/${room.id}/items?language=${encodeURIComponent(language)}`)
      if (response.ok) {
  ...
  const handleConfirmMove = async () => {
    ...
    try {
      const response = await fetch(`/api/warehouse/items/${selectedItemForMove.id}/move`, {
        method: 'PUT',
        headers: {
          'Content-Type': 'application/json',
        },
      ...
      if (response.ok) {
        ...
        if (selectedRoomForDetail) {
          const roomResponse = await fetch(`/api/warehouse/rooms/${selectedRoomForDetail.id}/items`)
          if (roomResponse.ok) {
  ...
  const handleViewHistory = async (item: any) => {
    try {
      const response = await fetch(`/api/warehouse/items/${item.id}/history`)
      if (response.ok) {
  ...
  const handleAddRoom = async (e: React.FormEvent) => {
    ...
    try {
      const response = await fetch('/api/warehouse/rooms', {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
        },
  ...
  const handleAddCabinet = async (e: React.FormEvent) => {
    ...
    try {
      const response = await fetch('/api/warehouse/cabinets', {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
        },
  ...
  const handleDeleteRoom = async (roomId: string, roomName: string) => {
    ...
    try {
      const response = await fetch(`/api/warehouse/rooms/${roomId}`, {
        method: 'DELETE',
      })
  ...
  const handleUpdateRoom = async (e: React.FormEvent) => {
    ...
    try {
      const response = await fetch(`/api/warehouse/rooms/${editingRoom.id}`, {
        method: 'PATCH',
        headers: {
          'Content-Type': 'application/json',
        },
  ...
  const handleUpdateCabinet = async (e: React.FormEvent) => {
    ...
    try {
      const response = await fetch(`/api/warehouse/cabinets/${editingCabinet.id}`, {
        method: 'PATCH',
        headers: {
          'Content-Type': 'application/json',
        },
  ...
  const handleDeleteCabinet = async () => {
    ...
    try {
      const response = await fetch(`/api/warehouse/cabinets/${cabinetToDelete.id}`, {
        method: 'DELETE',
        credentials: 'include',
      })
              ...
              <div className="flex justify-between items-center mb-4">
                ...
                <div className="flex space-x-2">
                  <button 
                    onClick={async () => {
                      try {
                        const response = await fetch('/api/warehouse/cleanup-duplicates', { method: 'POST' })
                        const result = await response.json()
                        if (response.ok) {
                  ...
                  <button 
                    onClick={async () => {
                      try {
                        ...
                        const response = await fetch('/api/warehouse/cleanup-category-duplicates', { method: 'POST' })
