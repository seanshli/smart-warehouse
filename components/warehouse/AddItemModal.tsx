...
export default function AddItemModal({ onClose }: AddItemModalProps) {
  ...
  const fetchCategories = async () => {
    ...
    try {
      const response = await fetch(`/api/warehouse/categories?householdId=${household.id}&language=${currentLanguage}`)
      if (response.ok) {
  ...
  useEffect(() => {
    const fetchCabinets = async () => {
      ...
      try {
        const response = await fetch(`/api/warehouse/cabinets?roomId=${formData.room}`)
