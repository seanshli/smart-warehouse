...
export default function CategoryManagement() {
  ...
  const fetchCategories = async () => {
    try {
      ...
      const url = `/api/warehouse/categories${params.toString() ? '?' + params.toString() : ''}`
  ...
  const handleDeleteCategory = async (categoryId: string, categoryName: string, itemCount: number) => {
    ...
    try {
      const response = await fetch(`/api/warehouse/categories/${categoryId}`, {
        method: 'DELETE',
      })
  ...
  const handleUpdateCategory = async (e: React.FormEvent) => {
    ...
    try {
      const response = await fetch(`/api/warehouse/categories/${editingCategory.id}`, {
        method: 'PATCH',
        headers: {
          'Content-Type': 'application/json',
        },
  ...
  const handleConfirmMove = async () => {
    ...
    try {
      const response = await fetch(`/api/warehouse/categories/${movingCategory.id}/move`, {
        method: 'PATCH',
        headers: {
          'Content-Type': 'application/json',
        },
