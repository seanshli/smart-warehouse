...
配網成功後：
1. **Tuya, Midea, Philips, Panasonic**：系統會自動填充設備信息到添加設備表單
2. **ESP 設備**：需要手動添加設備（在 MQTT Broker 上確認設備已連接後）
3. 確認設備信息
4. 選擇房間（可選）
5. 點擊 **Add Device** 完成添加
...
**配網模式**：
- **SmartConfig（ESP-TOUCH）**：通過 UDP 廣播發送 Wi-Fi 配置
  - 設備指示燈快速閃爍
  - 需要 ESP-TOUCH 工具或手機 App
  - 或使用本地工具通過 UDP 發送配置
- **AP 模式**：設備創建熱點，通過網頁配置
  - 設備創建熱點（ESP_XXXXXX）
  - 連接設備熱點
...
- 使用 2.4 GHz Wi-Fi 網絡
- 設備距離路由器 1-2 米內
- SmartConfig 需要本地工具或手機 App（瀏覽器無法直接發送 UDP）
...
- 可以在 MQTT Broker 上查看設備是否已連接
- 配網完成後需要手動添加設備（系統不會自動填充）

**推薦工具**：
- **ESP-TOUCH**：官方配網工具（Android/iOS）
- **ESP8266 SmartConfig**：開源配網工具
- **本地 UDP 工具**：用於發送 SmartConfig 數據包
